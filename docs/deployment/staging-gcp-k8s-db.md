...
**GHCR (образы `ghcr.io/...`):** в environment **staging** нужен секрет **`GHCR_READ_PACKAGES_TOKEN`** — PAT с правом **`read:packages`** (classic) или у fine-grained — **Packages → Read** для нужных репозиториев. В `kubectl create secret docker-registry` поле **password** = сам PAT, **username** = **GitHub login владельца PAT** (не название организации и не случайная строка). Workflow по умолчанию берёт **`github.actor`**; если при этом **403** на pull, задайте **`GHCR_DOCKER_USERNAME`** явно (логин пользователя, который выпустил PAT). У организации с **SSO** для PAT нажмите **Authorize**. Проверка с машины: `docker logout ghcr.io && docker login ghcr.io -u <логин> -p <PAT>` затем `docker pull ghcr.io/<repository_owner>/web:latest` — должен быть успех. В **Deploy to Staging** после записи секрета в кластер выполняется запрос к `ghcr.io/token` с теми же данными: если шаг падает с **HTTP ≠ 200**, PAT или username всё ещё неверны. Перед Helm workflow создаёт **`exponat-backend-env`** из **`exponat-postgres-auth`** / **`exponat-redis-auth`**, если секрета ещё нет (`infrastructure/k8s/ensure_exponat_backend_env.py`).
