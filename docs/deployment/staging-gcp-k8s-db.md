...
**GHCR (образы `ghcr.io/...`):** в environment **staging** нужен секрет **`GHCR_READ_PACKAGES_TOKEN`** — PAT с **`read:packages`** (classic) или fine-grained с **Packages → Read**. **Username** в docker-secret = **GitHub login владельца PAT**; по умолчанию workflow подставляет **`github.actor`**, при 403 задайте **`GHCR_DOCKER_USERNAME`**. У org с **SSO** — **Authorize** для токена. **Важно:** если у Deployment **`imagePullPolicy: IfNotPresent`**, поды часто **не обращаются к GHCR** (образ уже на ноде), а при **`Always`** — запрос идёт **каждый раз**. Поэтому 403 «только у web» часто означает не «отдельная поломка пакета web», а то, что **единственный сервис с Always упирался в OAuth**, а остальные жили с кэша; **новый PAT не обязателен** — сначала проверьте логин, нет ли лишнего пробела/переноса в секрете GitHub, и что OAuth-проверка в workflow (шаг после `ghcr-credentials`) проходит. Проверка вручную: `docker login ghcr.io` + `docker pull ghcr.io/<repository_owner>/web:latest`. Перед Helm workflow создаёт **`exponat-backend-env`** из **`exponat-postgres-auth`** / **`exponat-redis-auth`**, если секрета ещё нет (`infrastructure/k8s/ensure_exponat_backend_env.py`).
