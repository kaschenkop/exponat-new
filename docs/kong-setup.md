...
1. Поднимите основной стек в корне репозитория (нужны Redis и сервисы за прокси):

   ```bash
   docker compose up -d
   ```

2. Запустите Kong (сеть Docker Compose по умолчанию для каталога `exponat` — `exponat_default`):
...
3. Проверьте Admin API:

   ```bash
   curl http://localhost:8001/status
   ```

4. Направьте фронтенд на Kong (единая точка входа):

   - `NEXT_PUBLIC_API_BASE_URL=http://localhost:8000`
   - `NEXT_PUBLIC_PROJECTS_API_URL=http://localhost:8000`

   Пути: dashboard — `/api/dashboard`, projects — `/api/projects`, budget — `/api/v1/budgets` (см. `infrastructure/kong/kong.yml`).
